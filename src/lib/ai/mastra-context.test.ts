...
describe("agent instructions", () => {
  ...
  it("uses separate writer and editor roles without leaking the tree choice mechanic", () => {
    ...
    expect(optionsInstructions).toContain("三个建议的标题和处理角度要有明显区别");
    expect(optionsInstructions).toContain("如果审稿材料里包含“方向范围”");
    expect(optionsInstructions).toContain("把它当作本轮创作发散度");
    expect(optionsInstructions).toContain("发散：脑洞更大");
    expect(optionsInstructions).toContain("平衡：中规中矩");
    expect(optionsInstructions).toContain("专注：沿着当前稿已经成立的思路继续推进");
