...
Your job is to help a creator clarify intent, answer the most useful next question, and grow a seed idea toward a publishable social media draft.
...
Before proposing options, infer the one question that would help the creator most from the seed, current draft, user choice, path history, unused options, and selected skills.
Use the seed, draft, user choice, path history, unused options, and selected skills to choose one question and three possible answers.
...
One answer may be a finish choice when the draft is mature enough for light finishing; it still leads to an updated draft.
...
Generate the draft result for the current selected answer.
Use the seed, current draft, selected answer, path history, folded choices, learned preferences, and selected skills as writing context.
Apply the selected answer according to selected skills and the current draft state.
...
还没有选择答案。请先判断 seed 和当前草稿最需要创作者澄清、选择或推进什么，再基于已选技能生成一个最有帮助的问题和三个答案。
...
export function formatEnabledSkills(skills: Skill[]) {
  if (skills.length === 0) {
    return [
      "暂无已选技能。请基于 seed、草稿和用户选择继续判断创作下一步。",
...
判断结果体现在 roundIntent、三个答案和草稿生成里。

# 创作状态
作品类型与输出结构：
${parts.artifactContext || "作品类型：社媒内容。按默认社交媒体草稿结构输出。"}
...
# 生成要求
返回下一轮输出。roundIntent 要是当前最值得回答的一个问题；选项要贴合当前 seed、草稿进展、用户选择和已选技能，并写成对这个问题的三个可选答案。
必须遵守“作品类型与输出结构”里的字段、格式和交付要求。
按当前作品需要使用已选技能。
每次生成都要遵守所有已选技能的提示词。
把适合当前草稿状态的技能转化成下一步判断。
先按已选技能判断当前草稿状态、改动幅度和下一步问题。
...
专注：同一个问题下，沿当前稿已经成立的思路继续推进，优先补清楚、写顺、写实，不主动改换主题、读者、前提或基本结构。
选项以创作决策或回答口径为主；当当前草稿接近完成时，也可以包含轻量收尾项。
