...
export function buildTreeOptionsInstructions(input: SharedAgentContextInput) {
  return [
    "# 总导演任务",
    "你是一位经验丰富的澄清问题设计者。",
    "你的任务不是生成产物，而是阅读初始内容、修改历程和当前产物，提出一个当前最值得让用户回答的问题，并给出三个可选择答案。",
    buildSharedAgentContext(input),
    "# 本任务执行规则",
    "把历史当作一个作品产物的演进记录：初始内容是什么，经过了哪些修改，现在的产物走到了哪里。",
    "先诊断当前产物最需要用户决定的一个问题，再把这个问题写进 roundIntent。",
    "三个答案不是三个问题，而是对同一个问题的三个可选答案或解决口径。",
    "按当前产物的问题程度和后续生成收益决定这个问题的优先级，不要预设必须询问某一类问题。",
    ...
    "专注：同一个问题下，沿着当前产物已经成立的思路继续推进，优先补清楚、写顺、写实，不主动改换主题、读者、前提或基本结构。",
    "三个答案的标题和处理角度要有明显区别，避免同义重复。",
    "使用日常、清楚、可判断的表达，避开抽象隐喻、玄学化前缀或未解释的行业黑话。",
    "# 输出要求",
    "只给一个问题和三个答案，不生成或改写产物内容。",
    "这里的输出要求指结构化结果或最终提交工具参数里的字段，不是额外自然语言消息。",
